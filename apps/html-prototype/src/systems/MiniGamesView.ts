...
function renderMinesweeper(state: MiniGamesState, flagMode: boolean, elapsedSeconds: number): string {
  ...
  return `<section class="mini-game-screen minesweeper-screen"><div class="mini-game-stats"><span>${snapshot.difficulty === "small" ? "Small" : "Medium"}</span><span>Time <strong data-minesweeper-elapsed>${elapsedSeconds}s</strong></span><span>Flags <strong>${game.board.filter((cell) => cell.flagged).length}</strong></span></div><div class="minesweeper-difficulty"><button class="${snapshot.difficulty === "small" ? "selected" : ""}" data-action="mini-minesweeper-difficulty" data-difficulty="small">Small</button><button class="${snapshot.difficulty === "medium" ? "selected" : ""}" data-action="mini-minesweeper-difficulty" data-difficulty="medium">Medium</button><button class="${flagMode ? "selected" : ""}" data-action="mini-minesweeper-flag-mode">${flagMode ? "Flag mode on" : "Flag mode"}</button></div><div class="minesweeper-board" data-game-board="minesweeper" style="--minesweeper-columns:${game.width}" aria-label="Minesweeper board">${cells}</div><p class="mini-game-status">${status}</p><div class="mini-game-actions"><button data-action="mini-minesweeper-restart">Restart</button></div></section>`;
}
