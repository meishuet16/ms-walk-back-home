...
export const finalDreamMusic = "assets/audio/我好想你伴奏版Music.mp3";
...
type AppPrototype = { enterCurrentMemory?: () => Promise<void>; previewDoor?: (door: FinalDreamDoor) => void };
let activePresentation: FinalDreamPresentation | null = null;

function isFinalDreamDoor(door: { chapterId?: string } | null | undefined): boolean { return door?.chapterId === FINAL_DREAM_CHAPTER_ID; }
...
function finishFinalDreamIntoForest(app: AppLike): void {
  ...
  if (!audio || !finishReturnToForest || !audio.setScene) {
    audio?.stop?.(); audio?.setScene?.("forest"); audio?.setLoop?.(true); void audio?.ensurePlaying?.(); app.autosave?.(); return;
  }
  if (!(audio.isCurrentTrack?.(finalDreamMusic) ?? false)) { finishReturnToForest.call(app); return; }
  ...
  if (originalSetTrack) audio.setTrack = (src, autoPlay) => {
  ...
  try { finishReturnToForest.call(app); }
  finally { audio.setScene = originalSetScene; if (originalSetTrack) audio.setTrack = originalSetTrack; }
  audio.setLoop?.(false);
  let unsubscribe: () => void = () => {};
  unsubscribe = audio.onEnded?.(() => {
    unsubscribe();
    if (app.scene !== "forest" || !(audio.isCurrentTrack?.(finalDreamMusic) ?? false)) return;
    originalSetScene("forest"); audio.setLoop?.(true); void audio.ensurePlaying?.();
  }) ?? (() => {});
}
...
export function installFinalDreamBridge(prototype: AppPrototype): void {
  ...
  prototype.enterCurrentMemory = async function(this: AppLike): Promise<void> {
    ...
    this.audio?.stop?.(); this.audio?.setTrack?.(finalDreamMusic, false); this.audio?.setLoop?.(false); void this.audio?.ensurePlaying?.();
    const presentation = new FinalDreamPresentation({
      root: this.root, overlay: this.overlay, stage: this.stage,
      onComplete: () => {
        activePresentation = null;
        try { localStorage.setItem("walk.final-dream.seen", "1"); } catch { /* replay still works */ }
  ...
  if (originalPreviewDoor) prototype.previewDoor = function(this: AppLike, door: FinalDreamDoor): void {
    if (!isFinalDreamDoor(door)) { originalPreviewDoor.call(this, door); return; }
    this.currentDoor = door; void this.enterCurrentMemory?.();
  };
}
