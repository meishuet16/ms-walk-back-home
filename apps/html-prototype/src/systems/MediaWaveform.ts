...
export type DecodedWaveform = { durationMs: number; peaks: WaveformPeak[] };
...
export async function decodeMediaWaveform(file: File, sampleCount = 1200, signal?: AbortSignal): Promise<DecodedWaveform> {
  ...
  try {
    ...
    return { durationMs: Math.max(1, Math.round(buffer.duration * 1000)), peaks: reduceAudioPeaks(channels, sampleCount) };
  } catch (error) {
