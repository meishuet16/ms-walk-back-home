...
type AppLike = { personalPlayer?: PlayerWithVisibility; recordsPanelOpen?: boolean; room?: { vinylPlaying?: boolean }; save?: { savePersonalPlayer?: (state: unknown) => void }; overlay?: HTMLElement; showToast?: (message: string) => void; preserveRecordsScroll?: () => void; showRecords?: () => Promise<void>; allPersonalTracks?: () => Track[]; visibleMusicTracks?: () => Track[]; currentPersonalTrack?: () => Track | null; selectVinyl?: (id: string, announce?: boolean) => Promise<void>; playAdjacentPersonalTrack?: (direction: -1 | 1) => Promise<void>; handlePersonalTrackEnded?: () => Promise<void>; handleClick?: (event: Event) => void; autosave?: () => void };
...
export const developerHiddenRecordIds = new Set<string>([
  "audio-music", // 我好想你 (and existing Music-suffixed soundtrack IDs)
...
function injectVisibilityControls(app: AppLike): void {
  ...
  overlay.querySelectorAll<HTMLElement>("details").forEach((details) => { const summary = details.querySelector("summary"); if (summary?.textContent?.trim().startsWith("Sort:")) details.classList.add("records-sort-menu-anchor"); });
...
export function installRecordsLibraryVisibilityBridge(prototype: AppLike): void {
  ...
  const originalClick = prototype.handleClick; if (originalClick) prototype.handleClick = function(this: AppLike, event: Event): void {
    ...
    if (action === "hide-built-in-record" || action === "restore-built-in-record") { if (!this.personalPlayer) return; const trackId = target?.dataset.track ?? ""; const track = this.allPersonalTracks?.().find((item) => item.id === trackId); if (!trackId || track?.source !== "built-in" || isDeveloperHiddenRecord(trackId)) return; this.personalPlayer = action === "hide-built-in-record" ? hideBuiltInRecord(this.personalPlayer, trackId) : restoreBuiltInRecord(this.personalPlayer, trackId); persistVisibility(this); this.showToast?.(action === "hide-built-in-record" ? "Record hidden from your library" : "Record restored to your library"); void this.showRecords?.(); return; }
    originalClick.call(this, event);
  };
  prototype.playAdjacentPersonalTrack = async function(this: AppLike, direction: -1 | 1): Promise<void> { this.preserveRecordsScroll?.(); const tracks = personalPlaybackCandidates(this.allPersonalTracks?.() ?? [], this.personalPlayer); const nextId = nextTrackId(tracks.map((track) => track.id), this.currentPersonalTrack?.()?.id, direction, Boolean(this.personalPlayer?.shuffleEnabled)); if (nextId) await this.selectVinyl?.(nextId); };
  prototype.handlePersonalTrackEnded = async function(this: AppLike): Promise<void> { const player = this.personalPlayer; if (!player?.playing) return; const tracks = personalPlaybackCandidates(this.allPersonalTracks?.() ?? [], player); const selectedIsAvailable = Boolean(player.selectedTrackId && tracks.some((track) => track.id === player.selectedTrackId)); if (player.repeatOne && player.selectedTrackId && selectedIsAvailable) { await this.selectVinyl?.(player.selectedTrackId, false); return; } const nextId = nextTrackId(tracks.map((track) => track.id), player.selectedTrackId, 1, Boolean(player.shuffleEnabled)); if (!nextId) { player.playing = false; if (this.room) this.room.vinylPlaying = false; persistVisibility(this); return; } await this.selectVinyl?.(nextId, false); };
}
