...
type AppLike = {
  ...
  recordsMobile?: { actionTrackId?: string; layer?: string };
  activeRecordMenuTrackId?: string;
...
function injectVisibilityControls(app: AppLike): void {
  ...
  const chips = `<div class="records-library-visibility" role="group" aria-label="Record library filter">
    ${(["all", "personal", "built-in", "hidden"] as LibraryView[]).map((id) => `<button data-action="records-library-view" data-view="${id}" class="${view === id ? "selected" : ""}">${id === "all" ? "All" : id === "personal" ? "My Music" : id === "built-in" ? "Built-in" : `Hidden${hiddenCount ? ` (${hiddenCount})` : ""}`}</button>`).join("")}
  </div>`;
  overlay.querySelectorAll(".records-library, .records-song-tools").forEach((host) => {
    if (!host.querySelector(".records-library-visibility")) host.insertAdjacentHTML("afterbegin", chips);
  });
  ...
  overlay.querySelectorAll<HTMLElement>("[data-track], [data-record]").forEach((node) => {
    ...
    menu.insertAdjacentHTML("beforeend", `<button data-action="${isHidden ? "restore-built-in-record" : "hide-built-in-record"}" data-track="${trackId.replace(/&/g, "&amp;").replace(/\"/g, "&quot;")}" data-visibility-track="${trackId.replace(/&/g, "&amp;").replace(/\"/g, "&quot;")}">${isHidden ? "Restore to Records" : "Hide from Records"}</button>`);
  });
}
...
export function installRecordsLibraryVisibilityBridge(prototype: AppLike): void {
  ...
  if (originalVisible) {
    prototype.visibleMusicTracks = function(this: AppLike): Track[] {
  ...
  if (originalShow) {
    prototype.showRecords = async function(this: AppLike): Promise<void> {
  ...
  if (originalClick) {
    prototype.handleClick = function(this: AppLike, event: Event): void {
  ...
  prototype.playAdjacentPersonalTrack = async function(this: AppLike, direction: -1 | 1): Promise<void> {
    ...
    const nextId = nextTrackId(tracks.map((track) => track.id), current?.id, direction, Boolean(this.personalPlayer?.shuffleEnabled));
  ...
  prototype.handlePersonalTrackEnded = async function(this: AppLike): Promise<void> {
    ...
    if (player.repeatOne && player.selectedTrackId) {
      await this.selectVinyl?.(player.selectedTrackId, false);
      return;
    }
    const nextId = nextTrackId(tracks.map((track) => track.id), player.selectedTrackId, 1, Boolean(player.shuffleEnabled));
    if (!nextId) {
      player.playing = false;
      if (this.room) this.room.vinylPlaying = false;
      persistVisibility(this);
      return;
    }
