...
import { inAnyRect } from "./systems/CollisionSystem.js";
import { diaryEntryToDoor, makeDiaryEntry, parseDiaryImport } from "./systems/DiaryImport.js";
import { DialogueSystem } from "./systems/DialogueSystem.js";
import { resolveEnding } from "./systems/EndingResolver.js";
...
export class WalkBackHomeApp {
    ...
    openedDoors = new Set();
    choices = [];
    readMemories = new Set();
    scrapbook = new Set(["A warm door in the forest"]);
    favorites = new Set();
    timelineCompleted = new Set();
    selectedChapter = "Yumido Bread";
    settings = { rain: true, muted: false, volume: 0.45, compact: false, reducedMotion: false, musicEnabled: true, musicScene: "bakery" };
    room = { visits: 0, gifts: 0, outfit: "raincoat", diary: ["Muji put the water bottle on a tiny table and listened to the room breathe."], water: 2, warmth: 2, stickers: 0, letters: 0 };
    dialogue = new DialogueSystem(bakeryChapter.dialogue);
    ending = null;
    constructor(root) {
        this.root = root;
        root.innerHTML = `
    ...
    handleClick(event) {
        ...
        if (action === "open-tendency")
            this.showTendency();
        if (action === "open-room")
            this.showMujiRoom();
        if (action === "open-diary-editor")
            this.showDiaryEditor();
        if (action === "save-diary-entry")
            this.saveDiaryEntry(target.dataset.id);
        if (action === "edit-diary-entry")
            this.showDiaryEditor(target.dataset.id);
        if (action === "delete-diary-entry")
            this.deleteDiaryEntry(target.dataset.id ?? "");
        if (action === "import-diary-lines")
            this.importDiaryLines();
        if (action === "room-talk")
            this.roomTalk();
        if (action === "room-water")
            this.roomWater();
        if (action === "room-warm")
            this.roomWarm();
        if (action === "room-sticker")
            this.roomSticker();
        if (action === "room-letter")
            this.roomLetter();
        if (action === "room-gift")
            this.roomGift();
        if (action === "room-dress")
            this.roomDress();
        if (action === "room-diary")
            this.roomDiary();
        if (action === "compact")
            this.toggleCompact();
        ...
        if (action === "mute")
            this.toggleAudio();
        if (action === "music")
            this.toggleSceneMusic();
        if (action === "save")
            this.saveSlot(Number(target.dataset.slot));
        if (action === "load-slot")
            this.loadSlot(Number(target.dataset.slot));
        if (action === "delete-slot")
            this.deleteSlot(Number(target.dataset.slot));
        if (action === "ending")
        ...
        if (action === "enter-door") {
            ...
            if (doorId)
                this.currentDoor = this.allDoors().find((door) => door.id === doorId) ?? this.currentDoor;
            this.enterBakery();
        }
        if (action === "finish-memory")
            this.finishBakery();
        if (action === "choice")
            this.choose(target.dataset.choice ?? "");
    }
    loop(time) {
        ...
        if (this.scene === "forest")
            this.updateForest(input.x, input.y, dt);
        if (this.scene === "bakery")
            this.updateBakery(input.x, input.y, dt);
        this.draw(time);
    ...
    move(x, y, dt, boundsBlockers, objectBlockers) {
        ...
        if (moving) {
            ...
            if (!inAnyRect(next, [...boundsBlockers, ...objectBlockers]))
                this.player = next;
        }
        else {
            this.frame = 0;
    ...
    interact() {
        ...
        if (this.scene === "forest" && this.activeDoor)
            return this.previewDoor(this.activeDoor);
        if (this.scene === "bakery") {
            if (this.activeObject === "exit")
                return this.returnToForest();
            if (this.activeObject === "diary memory")
                return this.showDiaryMemory();
            if (this.activeObject === "Friend A") {
                if (!this.readMemories.has(this.currentMemoryKey()))
                    return this.showToast("Read the diary memory by the counter first");
                if (this.dialogue.complete())
                    this.resetBakeryDialogue();
                return this.showDialogue();
            }
            if (this.activeObject === "pastry")
                return this.inspectPastry();
    ...
    draw(time) {
        ...
        if (this.scene === "title")
            this.drawTitle(time);
        if (this.scene === "forest")
            this.drawScene(this.images.forest, time, "forest");
        if (this.scene === "bakery")
            this.drawScene(this.images.bakery, time, "bakery");
        if (this.scene === "ending")
            this.drawEnding();
        this.drawHud();
    }
    ...
    drawScene(image, time, kind) {
        ...
        if (kind === "bakery") {
            ...
            this.ctx.drawImage(this.images.friend, (600 - cameraX) * scale - 42 * scale, (430 - cameraY) * scale - 136 * scale, 84 * scale, 136 * scale);
        }
    ...
    drawMemorySpot(cameraX, cameraY, scale, time) {
    ...
    drawDoors(cameraX, cameraY, scale) {
    ...
    saveDiaryEntry(id = "") {
        const dateInput = this.overlay.querySelector("#diary-date");
        const titleInput = this.overlay.querySelector("#diary-title");
        const bodyInput = this.overlay.querySelector("#diary-body");
    ...
    importDiaryLines() {
        const importInput = this.overlay.querySelector("#diary-import");
        ...
        for (const entry of imported) {
            ...
            if (existing >= 0)
                this.diaryEntries[existing] = entry;
            else
                this.diaryEntries.push(entry);
        }
    ...
    deleteDiaryEntry(id) {
        ...
        if (entry) {
            ...
            if (this.currentDoor?.id === entry.id)
                this.currentDoor = null;
