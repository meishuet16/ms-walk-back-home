...
export function makeDiaryId(date, title) {
...
export function parseDiaryImport(text) {
    return text
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean)
...
export function makeDiaryEntry(date, title, body, id = makeDiaryId(date, title)) {
    return {
        id,
        date: date.trim(),
...
export function diaryEntryToDoor(entry, index) {
