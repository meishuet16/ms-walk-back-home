...
import {
  ...
  setTrimBoundary,
  timeAtPixel,
  timelineKeyboardStep,
  zoomTimeline
} from "../src/systems/WaveformModel.js";
...
test("zoom anchors the playhead and pan remains bounded", () => {
  ...
  assert.equal(timeAtPixel(zoomed, 0, 1000), zoomed.viewportStartMs);
...
test("peak reduction combines channels deterministically", () => {
  const peaks = reduceAudioPeaks([
    Float32Array.from([-1, -.5, .5, 1]),
    Float32Array.from([-.5, 0, .5, .5])
  ], 2);
  assert.deepEqual(peaks, [{ min: -.75, max: -.25 }, { min: .5, max: .75 }]);
});
...
test("keyboard precision follows modifier increments", () => {
  ...
  assert.equal(timelineKeyboardStep({ shiftKey: false, ctrlKey: false, metaKey: true }), 100);
});
